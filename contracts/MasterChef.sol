...
import '@hawk-protocol/swap-lib/contracts/math/SafeMath.sol';
...
interface IMigratorChef {
    // Perform LP token migration from legacy HawkSwap to HawkSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to HawkSwap LP tokens.
    // HawkSwap must mint EXACTLY the same amount of HawkSwap LP tokens or
    // else something bad will happen. Traditional HawkSwap does not
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        hawk.mint(address(syrup), hawkReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            string memory _symbol =  pool.lpToken.symbol();
            uint256 split;
            uint256 postSplit = _amount;
            if (!compareStrings(_symbol, 'HLP')) {
                split = _amount.mul(4).div(100);
                postSplit = _amount.sub(split);
                pool.lpToken.safeTransferFrom(address(msg.sender), address(this), split);
            }
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), postSplit);
            user.amount = user.amount.add(postSplit);
        }
    ...
    function safeHawkTransfer(address _to, uint256 _amount) internal {
        syrup.safeHawkTransfer(_to, _amount);
    }
